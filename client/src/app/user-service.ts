...
import {SetCurrentUser, RemoveCurrentUser, UserAdd, UserRemove, UserClear, ContactAdd, ContactRemove, ContactClear} from './actions/user.actions';

import {AddError, RemoveError, ClearErrors, AddMessage, RemoveMessage, ClearMessages} from './actions/message.actions';
...
export class UserService {
  ...
  constructor(private store: Store<{ users: UserStore[], messages: MessageStore[] }>) { 
  }
  ...
  login(username: string, password: string) {
    ...
    .catch(error => {
        ...
        if(error.response.data.msg) {
            errorObj.id = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
  ...
  updateUser(user) {
    ...
    return axios.put("http://localhost:5000/api/users", user, { headers: data})
    .then((res) => {
        const user = new User();
        user.id = res.data._id;
        user.name = res.data.name;
        ...
        messageObj.id = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
  ...
  deleteContact(contact) {
    ...
    return axios.delete(`http://localhost:5000/api/contacts/${contact._id}`, { headers: data})
        .then(res => {
            const user = new User();
